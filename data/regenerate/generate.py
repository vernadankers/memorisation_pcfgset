...
class MarkovTree():
    ...
    def string_argument(self):
        if not self.placeholders:
            candidate = tuple(random.choice(self.alphabet) for i in range(random.choice(self.lengths)))
            if not candidate in self.string_arguments:
                # make sure that the same string arguments do not occur
                self.string_arguments.add(candidate)
                return(candidate)
            else:
                return(self.string_argument())
        else:
    ...
    def evaluate_tree(self, tree, full_string):
        # Evaluate output
        if all(isinstance(item, str) for item in tree):
            return(tree)
        if tree[0] in self.unary_functions:
            return(tree[0](self.evaluate_tree(tree[1], full_string),
                           full_string,
                           self.is_overgen))
        elif tree[0] in self.binary_functions:
            return(tree[0](self.evaluate_tree(tree[1], full_string),
                           self.evaluate_tree(tree[2], full_string),
                           full_string,
                           self.is_overgen))
...
def count_overgen(tree):
    return tree.count("reverse ( echo") + \
        tree.count("prepend ( remove_first") + \
            tree.count("echo ( remove_first") + \
                tree.count("prepend ( reverse")
